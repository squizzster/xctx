...
Only modify the main code if you specifically we've agreed that, otherwise, feel free, go party, within the
Use the 
  ./experiments_tmp/<random_32_char_hex_string>/ space...
  ./experiments_tmp/`openssl rand -hex 16`>/ space...

## Project direction: no backward-compatibility burden

This repository is a pre-release/protocol-evolution codebase. Do not preserve
old behavior, old command names, old aliases, old file names, or compatibility
wrappers merely because they existed before.
...
- The production root command set is exactly `discover`, `observe`, `plan`,
