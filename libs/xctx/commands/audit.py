...
from xctx.domain.audit import audit_domain_level, audit_payload
...
def handle(store: dict, args: argparse.Namespace) -> int:
    ...
    ok = not any(not isinstance(check, dict) or check.get("status") == "fail" for check in payload.get("checks", []))
